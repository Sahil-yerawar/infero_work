...
int main(){
    ...
    while(cin>>n && n!=0){
      ...
      while(n--){
        ...
        cin>>x;
        if(y==n+1)pots.pb(x);
        else pots.pb(x+pots[pots.size()-1]);
        cout<<pots[pots.size()-1]<<" ";
      }
      cout<<endl;
      FenwickTree ft(pots.size()-1);
      for(int i=0;i<pots.size();i++){
        ft.adjust(pots[i],1);
      }
      ll l,r;
      cout<<ft.rsq(1,pots.size())<<endl;
      cout<<"Case "<<++o<<": "<<endl;
      while(cin>>s&&s!="END"){
        cin >> l >> r;
        if(s=="S")ft.adjust(l,r-pots[l-1]);
        else cout<<ft.rsq(l,r)<<endl;
      }
      cout<<" "<<endl;
    }
    return 0;
