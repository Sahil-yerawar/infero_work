...
ll data(ll x){
  ...
  ll rm = x-1;
  for(int i = 1;i< sz;i++){
    int j=rm/factorial(sz-i);
    rm = rm%factorial(sz-i);
    char t = numbers[j]+'0';
    permNum = permNum+t;
    numbers.erase(numbers.begin()+j);
    if(rm == 0)break;
  }
  for(size_t i = 0;i<numbers.size();i++){char t =numbers[i]+'0'; permNum = permNum+(t);}
  // numbers.clear();
  return toNum(permNum);
}
...
int main(){
    ...
    // vll::iterator p = max_element(all(req));
    // fill(*p);
