...
*/
bool show(string s,int i){
  if(s[i] == 'A' || s[i] == 'I'){
  if(i == 0){
    if(s.find_first_of('I',1)==string::npos)return true;
  }
  else if(i == s.size()-1){
    if(s.find_last_of('I',i-1)==string::npos)return true;
  }
  else{
    if(s.find_first_of('I',i+1)==string::npos && s.find_last_of('I',i-1)==string::npos)return true;
  }
  }
  return false;
}
