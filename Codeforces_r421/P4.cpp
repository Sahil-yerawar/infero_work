...
int main(){
    ...
    vector<ll> allperm;
    allperm.reserve(2*n-1);
    allperm.insert(allperm.end(),perm.begin()+1,perm.end());
    allperm.insert(allperm.end(),perm.begin(),perm.end());
    ll l =1;
    ll min = 0,ind = 0;
    for(ll i =0;i<n;i++){
      ll tot=0;
      ...
      l++;
      // }
      // if(tot < min){min = tot;ind = i;}
    }
    ll h = 1;
    for(ll j =n;j<(2*n)-1;j++){
      int x = min + abs(allperm[j]-n)-abs(allperm[j-n]-1);
      if(x < min){min = x;ind = h;}
      h++;
    }
    cout << min << " " << n-1-ind << endl;
    return 0;
}