...
// vector<vi> graph
int n,q,a,b,c;
int test(int n, int x, int y, int z){
  int tell[n] = {0};
  int a = x,b = z,cnt=0;
  if(a == y)tell[a] = 1;
  else{
...
int main(){
    ...
    while(q--){
      vi maxa;
      cin>>a>>b>>c;
      maxa.pb(test(n,a,b,c));
      cout<<1<<endl;
      maxa.pb(test(n,a,c,b));
      maxa.pb(test(n,c,a,b));
      maxa.pb(test(n,c,b,a));
      maxa.pb(test(n,b,c,a));
      maxa.pb(test(n,b,a,c));
      cout<<*max_element(all(maxa))<<endl;
    }
    return 0;
}