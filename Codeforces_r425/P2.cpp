...
*/
int n;
void match(string a, string b, string c){
  int x = -1,i,j;
  x = b.find('*');
  if(x != -1){
    if(x == 0)i = -1;
    ...
    if(x == b.size()-1)j = c.size();
    else{
      for(j = 0;b.size()-1-j != x;j++){
        if(b[b.size()-1-j]!= '?'){
          if(b[b.size()-1-j]!=c[c.size()-1-j]){
            cout<<"NO"<<endl;
            return;
          }
        }
        else{
          if(a.find(c[c.size()-1-j]) == string :: npos){
            cout<<"NO"<<endl;
            return;
          }
        }
    ...
    if(abs(i-j-2) == 1){cout<<"YES"<<endl;return;}
    else{
        for(int k = i+1;k<j;k++){
          if(a.find(c[k]) != string::npos){
            cout<<"NO"<<endl;
            return;
          }
        }
    ...
    if(b.size() != c.size()){cout<<"NO"<<endl;return;}
    for(i = 0;i<b.size();i++){
      if(b[i]!= '?'){
        if(b[i]!=c[i]){
          cout<<"NO"<<endl;
