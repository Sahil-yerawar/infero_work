...
int square[100][100];
vector <pii> coords;
void solve(int a[100][100],int b,int c,int d,int e){
  int y = c;
  for(int i = e-1;i<d;i++){
    a[n-d][i] = y;
    if(y%11==0)coords.pb(mp(n-d,i));
    y++;
  }
  for(int i = n-d+1;i<d;i++ ){
    a[i][d-1] = y;
    if(y%11==0)coords.pb(mp(i,d-1));
    y++;
  }
  for(int i = d-2;i>=e-1;i--){
    a[d-1][i] = y;
    if(y%11==0)coords.pb(mp(d-1,i));
    y++;
  }
  for(int i=d-2;i>e-1;i--){
    a[i][e-1] = y;
    if(y%11==0)coords.pb(mp(i,e-1));
    y++;
  }
  if(d-1 >= (int)ceil((double)n/2))solve(a,b,y,d-1,e+1);
  return;
}
int main(){
    ...
    cout<<endl;
    cout<<"Total Power points : "<<coords.size()<<endl;
    for(size_t i=0;i<coords.size();i++){
      cout<<'('<<coords[i].ff<<','<<coords[i].ss<<')'<<endl;
    }
    // cout<<endl;
    return 0;
}           