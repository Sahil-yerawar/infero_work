...
int main(){
    ...
    for(int i=0;i<n;i++){
      int l,r,cost;
      cin >> l >> r >> cost;
      if(r-l+1<x)data.pb(mp(mp(l,r),mp(r-l+1,cost)));
      if(data.size()!=0){
        for(int j=0;j<data.size();j++){
          if((r < data[j].ff.ff || l > data[j].ff.ss) && r-l+1 +data[j].ss.ff==x)minimal.pb(cost+data[j].ss.ss);
        }
      }
    }
    if(minimal.size()!=0) cout << *min_element(all(minimal) )<< endl;
    else cout<<-1<<endl;
    return 0;
}